...
run:
	make clean
	make build
	docker run --name ${APP_NAME} --network=zoo -dp 27020:27020 -e GRPC_PORT=27020 -e APP_NAME=${APP_NAME} -e ENV=local -e PG_DSN=postgres://email-service:email-service@email-service-db-local:31201/email-service?sslmode=disable ${APP_NAME}

PHONY: migrate-up
migrate-up:
	#docker exec -it practice psql -U postgres -c "create database crypto_service"
	goose -dir ./migrations postgres "postgres://email-service:email-service@localhost:26201/email-service?sslmode=disable" up
